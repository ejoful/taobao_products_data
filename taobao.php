...
//            $quatity_com_aver = $rate[6][0];
            ...
            $shop = '淘宝';
            if (stristr($item->comment_url, 'tmall'))
            {
                $shop = '天猫';
            }

            // 拼接product数据
            $data_string .= "(NULL, '$item->nid', '$item->category', '$model', '$name', '$shundian_price', '$item->nick', '$item->view_price', '$view_sales', '$comment_count', '0', '$credit', '$des', '$des_com_aver', '$attitude', '$attitude_com_aver', '$quatity', '$quatity_com_aver', '1', '$item->shopLink','$item->comment_url', '$shop'),";
...
// 如果有数据则插入到数据库中tb_product表
    if ($data_string) {
        ...
        $sql = "INSERT INTO `tb_product` (`id`, `nid`, `category`, `model`, `name`, `shundian_price`, `merchant_name`, `merchant_price`, `view_sales`, `reviews_count`, `is_coalition`, `credit`, `des`, `des_com_aver`, `attitude`, `attitude_com_aver`, `quatity`, `quatity_com_aver`, `good`, `shop_link`, `comment_url`, `shop`) VALUES $data_string";
//        echo "<br>\n\r" . $sql."<br>\n\r";
        $retval = mysql_query($sql, $con);
        if(!$retval) {
